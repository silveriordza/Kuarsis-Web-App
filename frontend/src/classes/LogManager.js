...
import {convertTo24HourFormat} from '../libs/Functions' 
...
LogThis  (logMessage, level = L3)  {
   if (level != OFF && this.LOG_LEVEL != OFF) {
      const LOCAL_LOG_LEVEL = this.LOG_LEVEL

      if (
         (isNaN(LOCAL_LOG_LEVEL) && LOCAL_LOG_LEVEL === level) ||
         this.LOG_LEVEL >= level
      ) {
         const currentDateTime = new Date()
         !this.logSettings
            ? console.log('%s: %s',  convertTo24HourFormat(currentDateTime), logMessage)
            : console.log(
                 '%s: %s, %s: %s',
                 convertTo24HourFormat(currentDateTime),
...
validateVars (logLevel, varNamesIn, vars)  {
   ...
   if (varNamesIn && varNamesIn != '' && vars && vars.length > 0) {
      ...
      if (varNames.length != vars.length) {
         this.LogThis(
            `Var names count does not match vars count varNamesIn=${varNamesIn}; varNames=${varNames}; vars=${this.j(
               vars,
            )}`,
            logLevel,
         )
      ...
      this.LogThis(
         `varNamesIn or vars is empty varNamesIn=${varNamesIn}; vars=${this.j(
            vars,
         )}`,
         logLevel,
      )
 ...
 */
 ...
 SetDebugSection(debugSectionNumber = this.LOG_LEVEL)  {
   this.LOG_LEVEL = debugSectionNumber
}
}
