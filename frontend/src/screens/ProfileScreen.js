...
const ProfileScreen = ({ location, history }) => {
  ...
  useEffect(() => {
    if (!userInfo) {
      LogThisLegacy(`ProfileScreen, useEffect, no userInfo available`);
      history.push("/sign-in");
    } else {
      LogThisLegacy(
        `ProfileScreen, useEffect, userInfo user=${JSON.stringify(
          user
        )}, success=${success}`
      );
      if (!user || !user.name || success || user._id !== userInfo._id) {
        ...
        LogThisLegacy(
          `ProfileScreen, useEffect, about to call getUserDetails with 'profile' as id`
        );
