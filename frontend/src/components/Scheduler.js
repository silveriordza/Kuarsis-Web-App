...
const Scheduler = ({ providerId, product }) => {
  ...
  LogThisLegacy(
    logSettings,
    `Entering to the Scheduler: providerId=${providerId}`
  );
  ...
  const [isEnoughNoticeInAdvanced, setisEnoughNoticeInAdvanced] =
    useState(true);
  ...
  const onActionBegin = (args) => {
    ...
    LogThisLegacy(
      logSettings,
      `Entering onActionBegin: args=${JSON.stringify(args)}`
    );
    ...
    if (args.requestType === "eventCreate") {
      ...
      LogThisLegacy(
        logSettings,
        `eventCreate: Entering, args=${JSON.stringify(args)}`
      );
      let scheduler = document.getElementById("scheduler").ej2_instances[0];
      let appointments = scheduler.getEvents();
      LogThisLegacy(
        logSettings,
        `eventChange: appointments=${JSON.stringify(appointments)}`
      );
      const _isAppointmentOveralapped = IsSlotAvailable(
        args.addedRecords[0],
        appointments
      );
      setisOverlapped(_isAppointmentOveralapped);

      const _isEnoughNoticeInAdvanced = IsEnoughNoticeInAdvanced(
        args.addedRecords[0]
      );
    ...
    if (args.requestType === "eventChange") {
      ...
      LogThisLegacy(
        logSettings,
        `eventChange: Entering, args=${JSON.stringify(args)}`
      );
  ...
  const IsSlotAvailable = (newEvent, eventList) => {
    ...
    let newEventList = eventList.filter(
      (e) => e.schedulerEventId !== newEvent.schedulerEventId
    );
    isOverlapped = newEventList.some(
      (eventAlreadyScheduled) =>
        (newEvent.StartTime >= eventAlreadyScheduled.StartTime &&
          newEvent.StartTime < eventAlreadyScheduled.EndTime) ||
        (newEvent.EndTime > eventAlreadyScheduled.StartTime &&
          newEvent.EndTime <= eventAlreadyScheduled.EndTime) ||
        (newEvent.StartTime === eventAlreadyScheduled.StartTime &&
          newEvent.EndTime === eventAlreadyScheduled.EndTime) ||
        (newEvent.StartTime < eventAlreadyScheduled.StartTime &&
          newEvent.EndTime > eventAlreadyScheduled.EndTime)
    ...
    if (isOverlapped) {
      LogThisLegacy(logSettings, `event is overlapped: true`);
      return true;
    }
    LogThisLegacy(logSettings, `event is overlapped: false`);
    return false;
  };

  const IsEnoughNoticeInAdvanced = (newEvent) => {
    const currentTime = new Date();
    const currentTimePlusHours = new Date(
  ...
  const onActionComplete = (args) => {
    ...
    LogThisLegacy(
      logSettings,
      ` args.requestType = ${JSON.stringify(
        args.requestType
      )}; args.data = ${JSON.stringify(
        args.data
      )}; args.deletedRecords=${JSON.stringify(
        args.deletedRecords
      )}; args.changedRecords=${JSON.stringify(args.changedRecords)}`
    );

    if (args.requestType === "eventCreated") {
      // This block is execute after an appointment create
      LogThisLegacy(logSettings, `eventCreated: Entering`);
      let scheduler = document.getElementById("scheduler").ej2_instances[0];
      let appointments = scheduler.getEvents();
      ...
      LogThisLegacy(
        logSettings,
        `eventCreated appointments=${JSON.stringify(appointments)}`
      );
    ...
    if (args.requestType === "eventChanged") {
      ...
      LogThisLegacy(logSettings, `eventChanged: Entering`);
      let scheduler = document.getElementById("scheduler").ej2_instances[0];
      let appointments = scheduler.getEvents();
      ...
      if (args.deletedRecords && args.deletedRecords.length > 0) {
        appointments = appointments.filter(
          (appt) =>
            !args.deletedRecords.some(
              (delEvent) => appt.schedulerEventId === delEvent.schedulerEventId
            )
        );
      }
      ...
      LogThisLegacy(
        logSettings,
        `eventChanged: Before updating Appointments index: appointments=${JSON.stringify(
          appointments
        )} args.data=${JSON.stringify(
          args.data
        )}; args.changedRecords=${JSON.stringify(
          args.changedRecords
        )}, args.deletedRecords=${JSON.stringify(args.deletedRecords)}`
      );
      ...
      if (
        ...
        LogThisLegacy(logSettings, `Recurrent event updated single ocurrence`);
        args.changedRecords[0].schedulerEventId = uuidv4();
        newAppointments = appointments.filter(
          (appt) => !args.changedRecords.some((dt) => appt.Id === dt.Id)
        );
        newAppointments = newAppointments.concat(args.changedRecords);
        LogThisLegacy(
          logSettings,
          `Recurrent event updated single ocurrence: newAppointments=${JSON.stringify(
            newAppointments
          )}`
        );
      } else {
        LogThisLegacy(
          logSettings,
          `Normal update for normal event (recurring or not)`
        );

        newAppointments = appointments.filter(
          (appt) =>
            !args.data.some(
              (dt) => appt.schedulerEventId === dt.schedulerEventId
            )
        );
        LogThisLegacy(
          logSettings,
          `Normal update for normal event (recurring or not): newAppointments=${JSON.stringify(
            newAppointments
          )}`
        );
      }
      ...
      LogThisLegacy(
        logSettings,
        `creating wholeSchedule: wholeSchedule=${JSON.stringify(wholeSchedule)}`
      );
      ...
      LogThisLegacy(
        logSettings,
        `Final schedule sending to DB for updatation: newSchedule=${JSON.stringify(
          newSchedule
        )}`
      );
      dispatch(updateScheduleDetails(newSchedule, product));
    ...
    if (args.requestType === "eventRemoved") {
      ...
      LogThisLegacy(
        logSettings,
        `eventRemoved: Before filtering Appointments: appointments=${JSON.stringify(
          appointments
        )}; args.data=${JSON.stringify(
          args.data
        )}; args.args.deletedRecords=${JSON.stringify(
          args.deletedRecords
        )}; args.changedRecords=${JSON.stringify(args.changedRecords)}`
      );
      let newAppointments = null;
      if (args.deletedRecords.length > 0) {
        ...
        const changedAppointmentIndex = appointments.findIndex(
          (appt) =>
            appt.schedulerEventId === args.changedRecords[0].schedulerEventId
        );
      ...
      LogThisLegacy(
        logSettings,
        `eventRemoved: After filtering Appointments: appointments=${JSON.stringify(
          newAppointments
        )}`
      );
      ...
      LogThisLegacy(
        logSettings,
        `eventRemoved: After contatenating Appointments: wholeSchedule=${JSON.stringify(
          wholeSchedule
        )}`
      );
      ...
      LogThisLegacy(
        logSettings,
        `eventRemoved: After updating newSchedule with schedule: newSchedule=${JSON.stringify(
          newSchedule
        )}`
      );
      ...
      LogThisLegacy(
        logSettings,
        `eventRemoved: After updating newSchedule with updated events: newSchedule=${JSON.stringify(
          newSchedule
        )}`
      );
    ...
    LogThisLegacy(logSettings, `eventCreated event Ended`);
  };
  ...
  useEffect(() => {
    ...
    LogThisLegacy(
      logSettings,
      `loading=${JSON.stringify(loading)}; schedule=${JSON.stringify(
        schedule
  ...
  return (
    <div className="schedulerClass">
      {console.log(
        `Scheduler, Rendering before loadingcheck: loading=${JSON.stringify(
          loading
      ...
      {loading && (!schedule ?? true) ? (
        <>
          {console.log(
            `Scheduler, Rendering Loading: loading=${JSON.stringify(
              loading
            )}; schedule=${JSON.stringify(schedule)}`
          )}
          <Loader />
        ...
        <>
          {console.log(
            `Scheduler, Rendering Start: loading=${JSON.stringify(
              loading
            )}; schedule.scheduleData=${JSON.stringify(schedule.scheduleData)} `
          )}
          {isOverlapped && (
            <div className="schedulerAlertMessage">
              <Message variant="danger">
                {
                  "Appoinments are overlapping, please select different date/times."
                }
              </Message>{" "}
            </div>
          )}
          {!isEnoughNoticeInAdvanced && (
            <div className="schedulerAlertMessage">
              <Message variant="danger">
                {
                  "Start Time must be at least 2 hours ahead of current time to give enough notice in advanced to provider."
                }
              </Message>{" "}
            </div>
          )}
      ...
      {LogThisLegacy(logSettings, `1 changing isInitialLoad.current=${isInitialLoad.current}`)}
      {isInitialLoad.current=false}
      {LogThisLegacy(logSettings, `2 changing isInitialLoad.current=${isInitialLoad.current}`)} */}
    </div>
  );
};
