...
const convertSchemaDefinitionToString = (modelSchema) => {
  ...
  const schemaDefinitionAsString = Object.fromEntries(
    Object.entries(modelSchema).map(([key, type]) => [key, "String"])
  );
  LogThis(
    log,
  ...
  try {
    ...
    dynamicModels.forEach((dynamicModel) => {
      ...
      for (let i = 0; i < schemaKeys.length; i++) {
        schemaObject[schemaKeys[i]] = { type: mongoose.Schema.Types.String };
        // LogThis(
        //   log,
...
const loadOneDynamicModelFromDB = async (modelName) => {
  ...
  try {
    ...
    if (dynamicModel) {
      ...
      throw new Error(`DynamicModel ${lowerCaseModel} can't be loaded from DB`);
    }
  } catch (error) {
    LogThis(
      log,
